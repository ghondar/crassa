...
async function create({ projectName, projectFolderName }) {
  ...
  console.log(
    `Creating project ${colorize(projectName).FgCyan()} in ${colorize(
      pathToUse
    ).FgCyan()}...`
  );

  const packageJSONTemplate = fs.readFileSync(
    path.resolve(__dirname, "./templates/package.template.json")
  );
  let packageJSON = replaceAll(
    packageJSONTemplate.toString(),
    "{-- project-name --}",
    projectName
  );
  ...
  const vscodeConfigTemplate = fs.readFileSync(
    path.resolve(__dirname, "./templates/settings.template.json")
  );
  const vscodeConfig = replaceAll(
    vscodeConfigTemplate.toString(),
    "{-- eslintrcPath --}",
    path.join(packageRootPath, ".eslintrc")
  );
  ...
  setTimeout(() => {
    ...
    childProcess.execSync(
      'git init . && git add . && git commit -m "Initialized app with crassa!"',
      { cwd: pathToUse }
    );
